    ...
    if [ $# -ne 0 ]; then
            ...
            case $opt in
              ...
              p) # PRECONFIGURED - Check the arguments
                 if [ "$OPTARG" == "1" ]; then
                     ...
                     argumentsToPass=",ArchiveZip,acpi,codecid,cpuinfo,devprop,dmi,edid,bootlogF,bootlogK,ioreg,kernelinfo,rcscripts,nvram,opencl,power,rtc,sip,smc,sysprof,Report"
                     wasDumpChosen=1
                 elif [ "$OPTARG" == "2" ]; then
                    # Run as root
                     ...
                     argumentsToPass=",ArchiveZip,acpi,codecid,cpuinfo,devprop,dmi,edid,bootlogF,bootlogK,ioreg,kernelinfo,rcscripts,nvram,opencl,rtc,sip,smc,sysprof,Report,privacy"
                     wasDumpChosen=1
                 elif [ "$OPTARG" == "4" ]; then
        ...
        if [ ! "$argumentsToPass" == "" ]; then
            ...
            if [ $wasDumpChosen -eq 1 ]; then
                ...
                declare -a rootDumps=(acpiFromMem codecid cpuinfo biosSystem biosVideo diskLoaderConfigs bootLoaderBootSectors diskVolumeXuid diskPartitionInfo firmmemmap memory kexts lspci)
                rootFlag=0
